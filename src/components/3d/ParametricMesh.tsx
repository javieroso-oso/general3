...
import { ParametricParams, ObjectType, printConstraints, SurfaceStroke } from '@/types/parametric';
  ...
  const { bodyGeometry, wireframeGeo, legGeometry, centeringLipGeometry, overhangColors, hasVertexColors, keyholeGeometries, cordHoleGeometry, wireframeLampGeometry } = useMemo(() => {
    ...
    for (let i = 0; i <= heightSegments; i++) {
      ...
      for (let j = 0; j <= segments; j++) {
        ...
        // This ensures preview and exported STL are identical
        ...
        // Track radius at each height (for overhang calculations)
        if (j === 0) {
          radiiAtHeight.push(r);
        }
